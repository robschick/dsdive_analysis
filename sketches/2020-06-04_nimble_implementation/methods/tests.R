...
# compute likelihood of observations, given model parameters
ld_nimble = cddive(x = sim.obs$depths, times = sim.obs$times, 
                   ...
                   T = c(492.3,t.stages,sim.obs$times[length(sim.obs$times)]),
                   log = TRUE)

# sim.obs$depths = rep(1, length(sim.obs$depths))
...
max(abs(obstx.mat[[3]]$vectors - obstx.mat_nimble[[3]][16 + 1:16,]))
